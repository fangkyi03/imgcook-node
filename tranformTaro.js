...
function createDiv(props, style, artboardImg, index) {
    ...
    return (Children, childArr, attrs) => {
        ...
        return `<View ${' '.repeat(index * 6)}className={styles.${props.className}}>\n${childrenText}</View>\n`
        // 
        // }else {
...
function createImage(props, style, index) {
    ...
    styles[props.className] = style;
    return Children => {
        return `<Image ${' '.repeat(index * 6)} className={styles.${props.className}} src={'${props['src']}'}/>\n`
    }
}
...
function createText(props, style, index) {
    ...
    return Children => {
        ...
        // }
        return `<Text ${' '.repeat(index * 6)} className={styles.${props.className}}>${props['text']}</Text>\n`
    }
}
...
function writeStyle(outfile) {
    ...
    fs.writeFileSync(path.join(outfile,'index.module.less'), format)
}

function tranformTaro (json,outfile) {
